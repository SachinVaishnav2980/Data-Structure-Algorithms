...
//this segment tree is for sum b/w two indexes.
public class SegmentTree {
    ...
    public Node constructTree(int []arr, int start, int end){
        if(start==end){
            //create a last leaf node
            Node leaf=new Node(start, end);
            leaf.data=arr[start];
            return leaf;
        }
        //create new node for that index
        Node node=new Node(start, end);
        int mid=(start+end)/2;
        //create a left leaf node and right leaf node
        node.left=this.constructTree(arr, start, mid);
        node.right=this.constructTree(arr, mid+1, end);

        //put data for that node 
